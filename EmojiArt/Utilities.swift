...
class ImageFetcher {
    ...
    /*
     ...
     If the fetch fails and a backup image is never set, the handler will neber be called.
     
     Note that you must keep a strong pointer to this object until the fetch finishes,
     otherwise the result if the fetch will be discarded and the handler will never be called.
    */
