...
class EmojiArtViewController: UIViewController, UIDropInteractionDelegate, UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDragDelegate, UICollectionViewDropDelegate {
    ...
    var emojiArt: EmojiArt? {
        get {
            if let url = emojiArtBackgroundImage.url { // if you don't have url we won't save you
    ...
    func dropInteraction(_ interaction: UIDropInteraction, performDrop session: UIDropSession) {
        ...
        session.loadObjects(ofClass: UIImage.self, completion: { images in
            if let image = images.first as? UIImage {
                self.imageFetcher.backup = image // use it as a backup
            }
        })
    }
        ...
        set {
            ...
            if let dropZone = self.dropZone, size.width > 0, size.height > 0 {
                scrollView?.zoomScale =
                    max(dropZone.bounds.size.width / size.width,
    ...
    var emojis = "🐸🐞🦋🐛🐢🐥🦢🦜🌴🌲🌳🌼🌸🌺🌻☀️☁️🌧⭐️🌙💫🔥🌈🚣🏼‍♀️🏊🏻‍♀️✈️⛵️🏕🏝🌋🏔⛱⛲️🏛🗽🧜🏼‍♀️🚶🏼‍♀️🚶🏻‍♂️".map { String($0) }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 1 {
            ...
            if let inputCell = cell as? TextFieldCollectionViewCell { // if it's text field
    ...
    private func dragItems(at indexPath: IndexPath) -> [UIDragItem] {
        if !addingEmoji, let attributedString = (emojiCollectionView.cellForItem(at: indexPath) as?
            ...
            dragItem.localObject = attributedString // we set it because when we drag inside our app, it's easier to use localObject to not deal with all multithreaded async tasks and all of that
            return [dragItem]
        } else {
            return []
        }
